...
// returns straight-line distance between two position vectors ({x, y}, {x', y'})
function distanceBetweenPositions(pos1, pos2) {
    return Math.sqrt((pos1.x - pos2.x) ** 2 + (pos1.y - pos2.y) ** 2)
}

// on every mousedown event, spawn the activeClickEffect
document.addEventListener("mousedown", event => {
    ...
    // if the target is a clickable element, spawn the clickEffect
    // at the middle of that element instead
    if (isClickFocusElement(event.target)) {
        ...
        // only do it if the actual click position isn't too far, 
        // otherwise it would look weird.
        // Too far is defined as size of the effect or 50 if that isn't defined yet.
        const positionDistance = distanceBetweenPositions(position, middlePosition)
        if (positionDistance < (activeClickEffectOptions.sizePx ?? 50)) {
...
// when the popup calls, update the settings
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    // (the popup is a really nice person obviously)
    if (request.action === "hi there please update thanks goodbye") {
        loadSettings()
    }
})
